...
def work_1
  file_lines = File.read('data/data_light.txt').split("\n")
  ...
  file_lines.each do |line|
    cols = line.split(',')
    users = users + [parse_user1(line)] if cols[0] == 'user'
    sessions = sessions + [parse_session1(line)] if cols[0] == 'session'
  end
  ...
  users.each do |user|
    ...
    user_object = User1.new(attributes: attributes, sessions: user_sessions)
